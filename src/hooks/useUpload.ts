...
export default function useUpload<T extends TfileType>(options: TOptions<T> = {} as TOptions<T>) {
  const { formData = {}, maxSize = 5 * 1024 * 1024, accept = ['*'], fileType = 'image', success, error: onError } = options
...
async function uploadFile({ tempFilePath, formData, onSuccess, onError, onComplete }: { tempFilePath: string; formData: Record<string, any>; onSuccess: (data: any) => void; onError: (err: any) => void; onComplete: () => void }) {
  uni.uploadFile({
