...
import { login as _login, refreshToken as _refreshToken, wxLogin as _wxLogin, getWxCode } from '@/api/login'
...
export const useTokenStore = defineStore(
  ...
  () => {
    ...
    if (typeof tokenInfo.value === 'string' || !tokenInfo.value) {
      tokenInfo.value = { ...tokenInfoState } as IAuthLoginRes
    }
    ...
    const setToken = (val: string) => {
      ...
      if (isSingleTokenMode) {
        ;(tokenInfo.value as any).token = val
      } else {
        ;(tokenInfo.value as any).accessToken = val
      }
    }
    ...
    const getToken = () => {
      ...
      if (isSingleTokenMode) {
        return (tokenInfo.value as any).token || ''
      } else {
        return (tokenInfo.value as any).accessToken || ''
      }
    }
     ...
     */
    async function _postLogin(token: string) {
      // 设置token
      setToken(token)
     ...
     * @returns 登录结果
     */
    const smsLogin = async (phoneNum: string, msgCode: string, type: string) => {
      // 调用短信登录API
      const { loginUsingPost, loginUsingCodePost } = await import('@/service/login')
      let loginApi = type === 'jdwl' ? loginUsingPost : loginUsingCodePost
      ...
      if (type === 'jdwl') {
        ...
        uni.navigateTo({
          url: '/pages-index/clientWrite/clientWrite',
        })
      }
